...
def test_caption_is_within_telegram_cap_and_informative():
    r = dt.score([4] * 16)
    cap = dt.result_caption(r, "Matt", "short verdict")
    assert len(cap) <= 1024
    assert "Matt" in cap
    assert "For fun, not a diagnosis." in cap
    assert "/6" in cap


def test_caption_trims_overlong_verdict_but_keeps_scores_and_citation():
    r = dt.score([4] * 16)
    huge = "blah " * 400                    # ~2000 chars, forces a trim
    cap = dt.result_caption(r, "Matt", huge)
    assert len(cap) <= 1024
    assert "For fun, not a diagnosis." in cap   # citation survives
    assert "🍽 Food" in cap and "/6" in cap      # scores survive


def test_image_prompt_is_safe_and_text_free():
    r = dt.score([5] * 16)
    p = dt.image_prompt(r).lower()
    assert "no text" in p and "no gore" in p
    assert r.biggest_ick_label in p


def test_item_image_prompt_is_safe_and_scene_specific():
    for item in dt.ALL_ITEMS:
        p = dt.item_image_prompt(item).lower()
        assert "no text" in p and "not gory" in p
        assert item.text.lower() in p        # the actual scene is described


@pytest.mark.parametrize("pos,total,expect_full", [
    (0, 16, False), (16, 16, True), (8, 16, None),
])
def test_progress_bar_bounds(pos, total, expect_full):
    bar = dt.progress_bar(pos, total)
    assert len(bar) == 10
    assert set(bar) <= {"▰", "▱"}
...
@pytest.mark.parametrize("score,filled", [(1.0, 0), (6.0, 10), (3.5, 5)])
def test_meter_maps_score_to_fill(score, filled):
    m = dt.meter(score)
    assert m == "▰" * filled + "▱" * (10 - filled)


def test_caption_includes_percentile_line_when_given():
    r = dt.score([4] * 16)
    cap = dt.result_caption(r, "Matt", "verdict", percentile=73)
    assert "Grosser than 73% of this chat" in cap
    assert len(cap) <= 1024


def test_caption_omits_percentile_line_when_none():
    r = dt.score([4] * 16)
    cap = dt.result_caption(r, "Matt", "verdict", percentile=None)
    assert "Grosser than" not in cap


def test_fallback_verdict_mentions_name_for_every_band():
    for answers in ([1] * 16, [2] * 16, [3] * 16, [4] * 16, [6] * 16):
        r = dt.score(answers)
        assert "Matt" in dt.fallback_verdict(r, "Matt")
